...
resnorm    = norm(RHST.S  (:,end))./(norm(S  (:))+eps) ...
           + norm(RHST.C  (:,end))./(norm(C  (:))+eps) ...
           + norm(RHST.X  (:,end))./(norm(rho(:))+eps) ...
           + norm(RHST.M  (:,end))./(norm(rho(:))+eps) ...
           + norm(RHST.MFD(:,end))./(norm(MFDSrc(:)/MFD0)+eps);
